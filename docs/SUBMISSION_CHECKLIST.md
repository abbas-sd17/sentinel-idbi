...
- [x] **Team formed** on HackToSkill: **ax3nr1x** (leader: Syed Abbas Raza)
- [x] **Track selected**: Track 04 - Default Prediction Model
- [x] **GitHub repo** is PUBLIC with all code pushed: https://github.com/abbas-sd17/sentinel-idbi
- [x] **Backend deployed** and `/health` returns `{"status":"ok"}`: https://sentinel-api-hv1o.onrender.com
- [x] **Frontend deployed** and loads portfolio dashboard: https://idbitrack4.netlify.app
- [x] **End-to-end verified**: dashboard → account detail (SMA/stage chips, decision panel) → batch upload validation all confirmed against the deployed stack
- [x] **3-minute demo video** recorded and uploaded: https://youtu.be/DLrS6FQ7Jcs
- [x] **PPT filled** using `scripts/fill_ppt.py` (15 slides, template headings preserved by fill_ppt.py)
- [x] **Screenshots added** to PPT Slide 10 (from the running prototype)

> ⏱️ **Warm the backend before judging/demo**: Render's free tier sleeps after 15 idle
> minutes — open https://sentinel-api-hv1o.onrender.com/health once, wait ~50 seconds,
> and the dashboard is instant after that.
...
3. Fill in:
   - **Deployment Link**: `https://idbitrack4.netlify.app` (frontend)
   - **GitHub Repo Link**: `https://github.com/abbas-sd17/sentinel-idbi`
   - **Prototype PPT**: Upload `Sentinel_Submission_Deck.pptx`
4. Click **Submit**
5. Deadline: **July 13, 2026, 11:59 PM IST** (portal showed EXTENDED — confirm the current close date on the portal)

### PPT Generation Command

```bash
pip install python-pptx
python scripts/fill_ppt.py   # defaults already carry the final team name, leader, and links
```
